...
public class DependencyGraphController {
     ...
     */
    @GetMapping("/{id}/graph")
    public ResponseEntity<?> getCodebaseGraph(@PathVariable Long id, HttpSession session) {
    ...
    public ResponseEntity<?> getClassDependencies(@PathVariable Long classId, HttpSession session) {
        User user = (User) session.getAttribute("user");
        if (user == null) {
            return ResponseEntity.status(401).body(Map.of("error", "Not authenticated"));
        }
    ...
    public ResponseEntity<?> getClassDependents(@PathVariable Long classId, HttpSession session) {
        User user = (User) session.getAttribute("user");
        if (user == null) {
            return ResponseEntity.status(401).body(Map.of("error", "Not authenticated"));
        }
