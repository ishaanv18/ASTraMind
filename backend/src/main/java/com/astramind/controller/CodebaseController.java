...
public class CodebaseController {
     ...
     */
    @GetMapping("/{id}")
    public ResponseEntity<?> getCodebase(@PathVariable String id, HttpSession session) {
        User user = (User) session.getAttribute("user");
        if (user == null) {
            return ResponseEntity.status(401).body(Map.of("error", "Not authenticated"));
        }
    ...
    public ResponseEntity<?> getCodebaseStatus(@PathVariable String id, HttpSession session) {
        User user = (User) session.getAttribute("user");
        if (user == null) {
            return ResponseEntity.status(401).body(Map.of("error", "Not authenticated"));
        }
     ...
     */
    @GetMapping("/{id}/files")
    public ResponseEntity<?> getCodebaseFiles(@PathVariable String id, HttpSession session) {
        User user = (User) session.getAttribute("user");
        if (user == null) {
            return ResponseEntity.status(401).body(Map.of("error", "Not authenticated"));
        }
    ...
    public ResponseEntity<?> getFileContent(@PathVariable String id, @PathVariable String fileId, HttpSession session) {
        User user = (User) session.getAttribute("user");
        if (user == null) {
            return ResponseEntity.status(401).body(Map.of("error", "Not authenticated"));
        }
     ...
     */
    @DeleteMapping("/{id}")
    public ResponseEntity<?> deleteCodebase(@PathVariable String id, HttpSession session) {
        User user = (User) session.getAttribute("user");
        if (user == null) {
            return ResponseEntity.status(401).body(Map.of("error", "Not authenticated"));
        }
