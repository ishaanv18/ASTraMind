...
public class CodeStructureController {
    ...
    public ResponseEntity<?> parseCodebase(@PathVariable String id, HttpSession session) {
    ...
    public ResponseEntity<?> getCodeStructure(@PathVariable String id, HttpSession session) {
        User user = (User) session.getAttribute("user");
        if (user == null) {
            return ResponseEntity.status(401).body(Map.of("error", "Not authenticated"));
        }
        ...
        try {
            ...
            List<Map<String, Object>> structure = classes.stream()
                    .map(cls -> {
                        ...
                        classData.put("fileId", cls.getFileId());

                        // Add methods (embedded documents don't have IDs)
                        ...
                        // Add fields (embedded documents don't have IDs)
    ...
    public ResponseEntity<?> getClasses(@PathVariable String id, HttpSession session) {
        User user = (User) session.getAttribute("user");
        if (user == null) {
            return ResponseEntity.status(401).body(Map.of("error", "Not authenticated"));
        }
