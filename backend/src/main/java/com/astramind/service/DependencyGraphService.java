...
public class DependencyGraphService {
     ...
     */
    public Map<String, Object> getCodebaseGraph(String codebaseId) {
        ...
        // Get relationships by iterating through classes
        List<CodeRelationship> relationships = new ArrayList<>();
        for (CodeClass codeClass : classes) {
            relationships.addAll(relationshipRepository.findBySourceClassId(codeClass.getId()));
        }
     ...
     */
    public List<CodeRelationship> getClassDependencies(String classId) {
        return relationshipRepository.findBySourceClassId(classId);
    }

    /**
     * Get dependents (classes that depend on this class)
     */
    public List<CodeRelationship> getClassDependents(String classId) {
    ...
    private Map<String, Object> buildEdge(CodeRelationship relationship) {
        if (relationship.getSourceClassId() == null) {
            return null;
        }

        Map<String, Object> edge = new HashMap<>();
        edge.put("id", "rel-" + relationship.getId());
        edge.put("source", "class-" + relationship.getSourceClassId());
